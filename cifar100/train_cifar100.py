...
bn14      = BatchNormalization(epsilon=epsilon, momentum=momentum, axis=1, name='bn14')(conv14)
act14     = Activation(relu6, name='act14')(bn14)

# conv15
conv15    = Conv2D(128, kernel_size, strides=(1, 1), padding='valid', data_format='channels_last',
            dilation_rate=(1, 1), activation=None, use_bias=True, kernel_initializer='glorot_uniform',
            bias_initializer='zeros', kernel_regularizer=None, bias_regularizer=None, activity_regularizer=None,
            kernel_constraint=None, bias_constraint=None, name='conv15')(act14)
pool15    = MaxPooling2D(pool_size=pool_size, name='pool15', data_format='channels_last')(conv15)
bn15      = BatchNormalization(epsilon=epsilon, momentum=momentum, axis=1, name='bn15')(pool15)
act15     = Activation(relu6, name='act15')(bn15)

# conv16
conv16    = Conv2D(128, kernel_size, strides=(1, 1), padding='valid', data_format='channels_last',
            dilation_rate=(1, 1), activation=None, use_bias=True, kernel_initializer='glorot_uniform',
            bias_initializer='zeros', kernel_regularizer=None, bias_regularizer=None, activity_regularizer=None,
            kernel_constraint=None, bias_constraint=None, name='conv16')(act15)
bn16      = BatchNormalization(epsilon=epsilon, momentum=momentum, axis=1, name='bn16')(conv16)
act16     = Activation(relu6, name='act16')(bn16)
# dropout12
dout11    = Dropout(0.25)(act16)
...
#Dense_11
dns11     = Dense(1024, activation=None, use_bias=True, kernel_initializer='glorot_uniform', bias_initializer='zeros',
...
##################################################################################################################################
# model generation
cfar100M  = Model(inputs=[input],outputs=[d_act12])

##################################################################################################################################
# model compilation
opt       = Adam()
earlystop = EarlyStopping(monitor='val_acc', patience=30, verbose=0, mode='auto')
cfar100M.compile(loss=keras.losses.categorical_crossentropy, optimizer=opt, metrics=['accuracy', top_3_accuracy])
...
##################################################################################################################################
# model visualization
plot_model(cfar100M, to_file='cfar100_baseline.png', show_shapes=True, show_layer_names=True)
...
cfarlog       = 'training_cifar100' + '_baseline_' + '.log'
csv_logger    = CSVLogger(cfarlog)
reduce_lr     = ReduceLROnPlateau(monitor='val_loss', factor=0.2,
                        patience=5, min_lr=0.0001)
tbCallBack    = keras.callbacks.TensorBoard(log_dir='./Graph_cfar100', histogram_freq=0, write_graph=True, write_images=True)

print ("############### Training CIFAR10 Dataset ################")
history1      = cfar100M.fit(X_train, Y_train,
...
##################################################################################################################################
# Test Scoring
score_cfar100b = cfar100M.evaluate(X_test, Y_test, verbose=0)
...
print('Test score cifar100 baseline:',    score_cfar100b[0])
print('Test accuracy cifar100 baseline:', score_cfar100b[1])

##################################################################################################################################
## model saving
cfar100M.save('cifar100_baseline.h5')
cfar100M.save('to_drop_layers_cifar100_baseline.h5')
cfar100M.save('golden_cifar100_baseline.h5')
cfar100M.save('read_cifar100_baseline.h5')
